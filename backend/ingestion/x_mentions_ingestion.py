#!/usr/bin/env python3
"""
X Mentions Ingestion Pipeline
Searches X for public posts mentioning Gauntlet AI and inserts them into posts
with is_mention=True. Uses the recent search endpoint (not the list feed).
"""
import json
import logging
import os
...
# Search query — configurable via env, sensible default
X_MENTION_QUERY = os.getenv(
    'X_MENTION_QUERY',
    '"Gauntlet AI" OR @GauntletAI OR gauntletai -is:retweet lang:en'
)

# Popular post thresholds (reuse same env vars as x_ingestion.py)
...
def x_get(url: str, params: dict, retries: int = 3) -> dict:
    auth = get_auth()
...
def send_popular_alert(post_data: dict, triggered_by: str, metric_value: int):
    """Send a Slack alert for a highly-engaged mention."""
    if not ALERT_CHANNEL or not SLACK_TOKEN:
        return
    ...
    try:
        ...
        log.error(f'Failed to send popular mention alert: {e}')


def check_popular_thresholds(cur, conn, post_id: str, post_data: dict):
    """Flag post in popular_posts if it crosses any threshold."""
    metrics  = post_data.get('metrics', {})
    views    = metrics.get('impression_count', 0)
    likes    = metrics.get('like_count', 0)
    reposts  = metrics.get('retweet_count', 0)
    ...
    try:
        ...
        conn.commit()
        log.info(f'Flagged mention {post_id} as popular ({triggered_by}: {metric_value})')
        send_popular_alert(post_data, triggered_by, metric_value)
    except Exception as e:
        log.error(f'Failed to flag popular mention {post_id}: {e}')
        conn.rollback()
...
def fetch_mention_tweets(since_id: Optional[str] = None) -> tuple[list, dict]:
    """
    Fetch recent tweets matching X_MENTION_QUERY via the search/recent endpoint.
    Returns (tweets, author_map).
    Paginates until we hit since_id or run out of results.
    """
    url = 'https://api.twitter.com/2/tweets/search/recent'
    params = {
        'query': X_MENTION_QUERY,
        'max_results': 100,
    ...
    all_tweets = []
    author_map: dict = {}
    max_pages = 10  # safety cap (~1000 tweets max per run)

    for page in range(max_pages):
        data = x_get(url, params)
        if 'error' in data or 'errors' in data:
            log.error(f'X search API error: {data}')
            break

        tweets = data.get('data', [])
        if not tweets:
            log.info(f'No tweets returned on page {page + 1} — done.')
            break
...
def run():
    ...
    # Load checkpoint — source key is 'x_mentions' (separate from 'x' list ingestion)
    cur.execute(
        'SELECT last_id, consecutive_failures FROM ingestion_checkpoints WHERE source = %s',
        ('x_mentions',)
    )
    row = cur.fetchone()
    ...
    try:
        log.info(f'Fetching X mentions (query: {X_MENTION_QUERY!r}, since_id: {since_id})')
        tweets, author_map = fetch_mention_tweets(since_id)
        log.info(f'Fetched {len(tweets)} mention tweets')

        if not tweets:
            log.info('No new mention tweets.')
        else:
            # Track latest tweet ID for next checkpoint
            ...
            if not new_posts:
                log.info('All fetched tweets already in DB (deduped).')
            else:
                BATCH = 50
                for i in range(0, len(new_posts), BATCH):
                    ...
                    try:
                        ...
                        log.error(f'Embedding failed for batch {i // BATCH + 1}: {e}')
                    ...
                    for post, embedding in zip(batch, embeddings):
                        try:
                            ...
                            if inserted_row:
                                check_popular_thresholds(cur, conn, str(inserted_row['id']), {
                                    'author': post['author'],
                    ...
                    conn.commit()
                    log.info(f'Inserted mention batch {i // BATCH + 1} ({len(batch)} posts)')

                log.info(f'X mentions ingestion complete: {len(new_posts)} new posts')

            # Update checkpoint with latest seen tweet ID
            cur.execute('''
            ...
            ''', ('x_mentions', latest_id, 'success'))
            conn.commit()

        # Also update checkpoint on zero-result runs
        if not tweets:
            cur.execute('''
                INSERT INTO ingestion_checkpoints (source, last_id, last_run_at, status, consecutive_failures)
                VALUES (%s, %s, NOW(), %s, 0)
        ...
        ''', ('x_mentions', 'failed', failures))
        conn.commit()
        log.error(f'X mentions ingestion failed: {e}')
        if failures >= 2:
            send_alert(
                f'🚨 MCC: X mentions ingestion has failed {failures} times in a row.\nError: {e}'
            )
        raise
    finally:
        cur.close()
